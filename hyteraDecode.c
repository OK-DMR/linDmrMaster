...
void decodeHyteraGpsTriggered(int radioId, int destId, struct repeater repeater, unsigned char data[300]) {
    ...
    syslog(LOG_NOTICE, "[%s]Decoded GPS data triggered(Hytera): LAT(%s) LONG(%s) SPEED(%s) HEADING(%s)",
           repeater.callsign, gpsData.latitude, gpsData.longitude, gpsData.speed, gpsData.heading);
...
void decodeHyteraGpsButton(int radioId, int destId, struct repeater repeater, unsigned char data[300]) {
    ...
    syslog(LOG_NOTICE, "[%s]Decoded GPS data button(Hytera): LAT(%s) LONG(%s) SPEED(%s) HEADING(%s)", repeater.callsign,
...
void decodeHyteraRrs(struct repeater repeater, unsigned char data[300]) {
    ...
    sprintf(SQLQUERY,
            "REPLACE into rrs (radioId,callsign,name,registerTime,onRepeater,unixTime) VALUES (%i,'%s','%s','%s','%s',%lu)",
            srcId, callsign, name, timeStamp, repeater.callsign, now);
