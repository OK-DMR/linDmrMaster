...
int initRepeater(struct sockaddr_in address) {
    ...
    db = openDatabase();
    sprintf(SQLQUERY,
            "SELECT repeaterId,callsign,txFreq,shift,hardware,firmware,mode,language,geoLocation,aprsPass,aprsBeacon,aprsPHG,autoReflector,intlRefAllow,reflectorTimeout,autoConnectTimer FROM repeaters WHERE ipAddress = '%s'",
            str);
    if (sqlite3_prepare_v2(db, SQLQUERY, -1, &stmt, 0) == 0) {
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            syslog(LOG_NOTICE, "Assigning %s %s %s %s %s %s %s autoref %i to repeater on pos %i from database [%s]",
                   repeaterList[i].callsign, repeaterList[i].hardware, repeaterList[i].firmware, repeaterList[i].mode,
                   repeaterList[i].txFreq, repeaterList[i].shift, repeaterList[i].language,
...
#pragma clang diagnostic ignored "-Wmissing-noreturn"
    for (;;) {
        ...
        if (FD_ISSET(sockfd, &fdService)) {
            ...
            if (memcmp(buffer, command, sizeof(command)) == 0) {  //See if what is received is a command or heartbeat
                ...
                switch (buffer[20]) {
                    case 0x10: {  //PTPP request received
                        ...
                        if (difftime(timeNow, repeaterList[repPos].lastPTPPConnect) < 10)
                            continue;  //Ignore connect request
                    ...
                    case 0x11: {  //Request to startup DMR received
                        ...
                        if (difftime(timeNow, repeaterList[repPos].lastDMRConnect) < 10)
                            continue;  //Ignore connect request
                        ...
                        if (repPos == 99) {  //If  not ignore the DMR request
                            syslog(LOG_NOTICE, "DMR request from repeater not in repeater list [%s-%i], ignoring", str,
                                   recPort);
                            continue;
                        }
                        //See if RDAC info from the repeater has already been received
                        if (!repeaterList[repPos].id > 0) { //If not ignore DMR request
                            syslog(LOG_NOTICE, "RDAC info not received from repeater yet [%s-%i], ignoring", str,
                                   recPort);
                            continue;
                        }
                        memcpy(response, buffer, n);
                        ...
                        sendto(sockfd, response, n + 1, 0, (struct sockaddr *) &cliaddr, sizeof(cliaddr));
                        syslog(LOG_NOTICE, "Assigned DMR device 1 to repeater [%s - %s]", str,
                        ...
                        if (repeaterList[repPos].dmrOnline) {  //If repeater is not offline, but we still get a request, just point it back to old thread
                            ...
                            syslog(LOG_NOTICE,
                                   "DMR request from repeater [%s - %s] already assigned a DMR port, not starting thread",
                                   str, repeaterList[repPos].callsign);
                        ...
                        syslog(LOG_NOTICE, "Re-directed repeater [%s - %s] to DMR port %i", str,
                               repeaterList[repPos].callsign, redirectPort);
                        break;
                    }
                    ...
                    case 0x12: {  ////Request to startup RDAC received
                        ...
                        repPos = findRepeater(cliaddr);
                        //cliaddr.sin_port=repeaterList[repPos].address.sin_port;
                        if (difftime(timeNow, repeaterList[repPos].lastRDACConnect) < 10)
                            continue;  //Ignore connect request
                        ...
                        if (repeaterList[repPos].rdacOnline) {  //If repeater is not offline, but we still get a request, just point it back to old thread
                            syslog(LOG_NOTICE,
                                   "RDAC request from repeater [%s - %s] already assigned a RDAC port, not starting thread",
                                   str, repeaterList[repPos].callsign);
                            repeaterList[repPos].rdacUpdateAttempts = 0;
            ...
            if ((memcmp(buffer + 4, ping, sizeof(ping)) == 0) && repeaterList[findRepeater(
                    cliaddr)].dmrOnline) {//Is this a heartbeat from a repeater on the service port ? And do we know the repeater ?
                memcpy(response, buffer, n);
                response[12]++;
...
int getMasterInfo() {
    ...
    db = openDatabase();
    sprintf(SQLQUERY,
            "SELECT ownName,ownCountryCode,ownRegion,sMasterIp,sMasterPort,priorityTGTS1,priorityTGTS2, priorityTimeout,eMail FROM sMaster");
    if (sqlite3_prepare_v2(db, SQLQUERY, -1, &stmt, 0) == 0) {
        if (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    sqlite3_finalize(stmt);
    syslog(LOG_NOTICE,
           "sMaster info: ownName %s, ownCountryCode %s, ownRegion %s, sMasterIp %s, sMasterPort %s priorityTGTS1 %i, priorityTGTS2 %i, priorityTimeout %i",
           master.ownName, master.ownCountryCode, master.ownRegion, master.sMasterIp, master.sMasterPort,
           master.priorityTG[1], master.priorityTG[2], master.priorityTimeout);

    sprintf(SQLQUERY,
            "SELECT servicePort, rdacPort, dmrPort, baseDmrPort, maxRepeaters, echoId,rrsGpsId,aprsUrl,aprsPort,echoSlot,baseRdacPort,masterDmrId FROM master");
    if (sqlite3_prepare_v2(db, SQLQUERY, -1, &stmt, 0) == 0) {
        if (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    syslog(LOG_NOTICE,
           "ServicePort %i rdacPort %i dmrPort %i baseDmrPort %i baseRdacPort %i maxRepeaters %i echoId %i echoSlot %i rrsGpsId %i software_ID %i",
...
void getLocalReflectors() {
    ...
    if (sqlite3_prepare_v2(db, SQLQUERY, -1, &stmt, 0) == 0) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            syslog(LOG_NOTICE, "Added reflector %i %s type %s", localReflectors[numReflectors].id,
                   localReflectors[numReflectors].name, localReflectors[numReflectors].type == 1 ? "intl" : "local");
            numReflectors++;
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            if (lineread = strtok(sMasterTS1, ",")) {
                ...
                while (lineread = strtok(NULL, ",")) {
                    if (master.sMasterTS1GroupCount > size)
                        sMasterTS1List = realloc(sMasterTS1List, (sizeof *sMasterTS1List) *
                                                                 master.sMasterTS1GroupCount);
                    if (strstr(lineread, "**")) { //If ** in talkgroup, this is a range of 100
            ...
            if (lineread = strtok(sMasterTS2, ",")) {
                ...
                while (lineread = strtok(NULL, ",")) {
                    if (master.sMasterTS2GroupCount > size)
                        sMasterTS2List = realloc(sMasterTS2List, (sizeof *sMasterTS2List) *
                                                                 master.sMasterTS2GroupCount);
                    if (strstr(lineread, "**")) {//If ** in talkgroup, this is a range of 100
            ...
            if (lineread = strtok(repTS1, ",")) {
                ...
                while (lineread = strtok(NULL, ",")) {
                    if (master.repTS1GroupCount > size)
                        repTS1List = realloc(repTS1List, (sizeof *repTS1List) * master.repTS1GroupCount);
                    if (strstr(lineread, "**")) {//If ** in talkgroup, this is a range of 100
            ...
            if (lineread = strtok(repTS2, ",")) {
                ...
                while (lineread = strtok(NULL, ",")) {
                    if (master.repTS2GroupCount > size)
                        repTS2List = realloc(repTS2List, (sizeof *repTS2List) * master.repTS2GroupCount);
                    if (strstr(lineread, "**")) {
            ...
            if (lineread = strtok(dynamicTS1, ",")) {
                ...
                while (lineread = strtok(NULL, ",")) {
                    if (master.dynTS1GroupCount > size)
                        dynTS1List = realloc(dynTS1List, (sizeof *dynTS1List) * master.dynTS1GroupCount);
                    if (strstr(lineread, "**")) {//If ** in talkgroup, this is a range of 100
            ...
            if (lineread = strtok(dynamicTS2, ",")) {
                ...
                while (lineread = strtok(NULL, ",")) {
                    if (master.dynTS2GroupCount > size)
                        dynTS2List = realloc(dynTS2List, (sizeof *dynTS2List) * master.dynTS2GroupCount);
                    if (strstr(lineread, "**")) {
