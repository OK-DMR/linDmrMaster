...
bool getRepeaterInfo(int sockfd, int repPos, struct sockaddr_in cliaddrOrg, sqlite3 *dbase) {
                ...
                while (lineread = strtok(NULL, ":")) {
                    ...
                    switch (select_str(lineread)) {
                        case 0:  //0 = repeaterId
                            ...
                            syslog(LOG_NOTICE, "Assigning id %i to repeater on pos %i [%s]", repeaterList[repPos].id,
                                   repPos, str);
                            sprintf(SQLQUERY, "SELECT repeaterId FROM repeaters WHERE repeaterId = %i",
                                    repeaterList[repPos].id);
                            if (sqlite3_prepare_v2(dbase, SQLQUERY, -1, &stmt, 0) == 0) {
                                if (sqlite3_step(stmt) != SQLITE_ROW) {
                                    syslog(LOG_NOTICE,
                                           "Repeater with id %i not known in database, removing from list pos %i [%s]",
                                           repeaterList[repPos].id, repPos, str);
    ...
    syslog(LOG_NOTICE, "Updating from RDAC %s %s %s %s %s %s to repeater on pos %i [%s]", repeaterList[repPos].callsign,
           repeaterList[repPos].hardware, repeaterList[repPos].firmware, repeaterList[repPos].mode,
           repeaterList[repPos].txFreq, repeaterList[repPos].shift, repPos, str);
    ...
    if (sqlite3_prepare_v2(dbase, SQLQUERY, -1, &stmt, 0) == 0) {
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            sqlite3_finalize(stmt);
            sprintf(SQLQUERY,
                    "UPDATE repeaters SET callsign = '%s', txFreq = '%s', shift = '%s', hardware = '%s', firmware = '%s', mode = '%s', currentAddress = %lu, lastRdacUpdate = '%s', ipAddress = '%s'  WHERE repeaterId = %i",
                    repeaterList[repPos].callsign, repeaterList[repPos].txFreq, repeaterList[repPos].shift,
                    repeaterList[repPos].hardware, repeaterList[repPos].
                            firmware, repeaterList[repPos].mode, (long) cliaddrOrg.sin_addr.s_addr, timeStamp, str,
                    repeaterList[repPos].id);
            sqlite3_exec(dbase, SQLQUERY, 0, 0, 0);
            sprintf(SQLQUERY,
                    "SELECT language,geoLocation,aprsPass,aprsBeacon,aprsPHG,autoReflector,intlRefAllow,reflectorTimeout,autoConnectTimer FROM repeaters WHERE repeaterId = %i",
                    repeaterList[repPos].id);
            if (sqlite3_prepare_v2(dbase, SQLQUERY, -1, &stmt, 0) == 0) {
                if (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    for (;;) {
        ...
        if (FD_ISSET(sockfd, &fdMaster)) {
            ...
            if (n > 2) {
                ...
                time(&pingTime);
                response[0] = 0x41;
                if (repPos != 99 && cliaddr.sin_addr.s_addr == cliaddrOrg.sin_addr.s_addr)
                    sendto(sockfd, response, n, 0, (struct sockaddr *) &cliaddr, sizeof(cliaddr));
                if (repPos != 99 && !repeaterList[repPos].rdacUpdated &&
                    cliaddr.sin_addr.s_addr == cliaddrOrg.sin_addr.s_addr &&
                    repeaterList[repPos].rdacUpdateAttempts < 10) {
            ...
            if (difftime(timeNow, pingTime) > 60) {
                ...
                if (repPos != 99) {
                    syslog(LOG_NOTICE, "PING timeout on RDAC port %i repeater %s, exiting thread [%s]", port,
                           repeaterList[repPos].callsign, str);
                    syslog(LOG_NOTICE, "Setting RDAC offline for repeater on position %i [%s]", repPos, str);
                    repeaterList[repPos].rdacOnline = false;
                } else {
                    syslog(LOG_NOTICE,
                           "PING timeout on RDAC port %i repeater already removed from list, exiting thread", port);
                }
                close(sockfd);
                pthread_exit(NULL);
            }
